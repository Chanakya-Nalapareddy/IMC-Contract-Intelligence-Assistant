...
def main():
    ...
    if len(files) > 1:
        raise RuntimeError("Keep only ONE file in v1/data/raw for this run:\n" +
    ...
    # Stream pages: write extracted text as we go, and feed into chunker
    ...
    with chunks_path.open("w", encoding="utf-8") as fout:
        for ch in tqdm(chunk_contract_from_pages(page_stream_and_save()),
                       desc="Chunking", unit="chunk"):
            total += 1
            rec = {
